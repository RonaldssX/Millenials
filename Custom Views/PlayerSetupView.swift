...
fileprivate let TextFieldIcon: UIImage = UIImage(named: "NameInput")!
...
class PlayerSetupView: UIView {    
    ...
    public var playerTextField: FlatTextField! {
        ...
        didSet {
            ...
            self.playerTextField.attributedPlaceholder = NSAttributedString(string: "Nome", attributes: [key.foregroundColor: UIColor.OffWhite.withAlphaComponent(0.5)])
            self.playerTextField.textAlignment = .center
            
            self.playerTextField.keyboardAppearance = .dark
            self.playerTextField.autocorrectionType = .default // <- esse merda que ta causando o leaking
            self.playerTextField.autocapitalizationType = .words
            
            self.playerTextField.font = UIFont.defaultFont(size: 16, weight: .light)
